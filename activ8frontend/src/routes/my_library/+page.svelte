...
const addFolder = async () => {
  ...
  const removeFolder = async (folderId) => {
    try {
      const response = await fetch(`http://localhost:8080/api/studyfolders/${folderId}`, {
        method: "DELETE",
        headers: {
          "Content-Type": "application/json",
        },
...
<div class="flex flex-col w-screen h-screen bg-[#ECF0F5] items-center">
  <nav class="flex w-full mt-4">
    ...
    <button on:click={toggleEditMode} class="border border-gray-500 rounded" style="width: 100px; height: 40px; position: absolute; right: 180px; top: 25px; margin-right: 10px;">
      {#if editMode}
        Done
      {:else}
        Edit folders
     ...
     <button on:click={addFolder} class="border border-gray-500 rounded" style="width: 100px; height: 40px; position: absolute; right: 300px; top: 25px;">Add folders</button>
              ...
              <div class="flex mt-4 items-center w-full">
                <div class="grid grid-cols-3 w-full h-full">
                  {#if responseData}
                    ...
                    >
                      {#if editMode}
                        <button class="absolute z-10 top-2 right-2 p-2 text-red-500" on:click={() => removeFolder(folder.id)}>
                          &#10006;
                        </button>
                      {/if}
